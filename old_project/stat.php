<?php
	...
	$stat_sql="select a.*,(select count(*) from vote where vote.vote_id = a.vote_id and vote.whendo between ? and ? ) votes,(select count(*) from message where message.player_uid = a.unique_id  and message.whendo between ? and ? ) number,(select count(*) from call_detail b where b.transfer_num = a.player_tel  and b.begintime between ? and ? ) calls from player a where  a.vote_id=0 and a.player_id<999 order by votes desc ,number desc,calls desc, a.unique_id desc limit {$offset},{$pageSize}";
